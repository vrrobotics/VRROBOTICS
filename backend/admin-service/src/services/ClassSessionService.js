...
const listForTeacher = async (teacherId) => {
    ...
    try {
        ...
        const allSids = rows.flatMap((r) => (Array.isArray(r.student_ids) ? r.student_ids : []));
        const names = await resolveUserNames(allSids);
        const classes = rows.map((r) => ({
            ...
            students: (Array.isArray(r.student_ids) ? r.student_ids : []).map((id) => ({ id: String(id), name: names[String(id)] || String(id) })),
        }));
        return { classes };
    } catch (err) {
