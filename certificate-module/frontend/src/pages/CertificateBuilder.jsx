...
import { getSettings, updateBuilder } from '../api/certificate';
...
//   - slide-in right sidebar with toggle button
//   - "Available Variable Data" badges
//   - "Add a new element" form (textarea / font radios / font size / Add / Save Template)
//   - Drag-to-position (+ remove) on each element
//   - Saves the canvas DOM HTML back via POST /admin/certificate/builder
...
// Matches the radio set in builder.blade.php (lines 177-180): Auto + Pinyon Script.
// Italianno + Miss Fajardose @import urls live in the inline <style> in the
// blade so they're available for any saved HTML that uses them, but the form
// only exposes Auto and Pinyon Script.
const FONT_FAMILIES = [
    { value: 'auto', label: 'Auto' },
    { value: 'Pinyon Script', label: 'Pinyon Script' },
];

// Default canvas size — same proportions as the PHP layout module.
...
export default function CertificateBuilder() {
    ...
    // New-element form state (mirrors the PHP "Add a new element" panel)
    ...
    useEffect(() => {
        const init = (s) => {
            ...
            // If parsing produced nothing (empty saved content), seed from default.
    ...
    // ---- Drag (functional equivalent of jQuery UI .draggable) ----
    ...
    // ---- Add / remove / variable insertion (mirrors PHP addElemToCertificate) ----
    const addElement = () => {
    ...
    // ---- Save (mirrors PHP saveTemplate -> POST builder content) ----
    const onSave = async () => {
        setSubmitting(true);
        setMessage(null);
        try {
            const html = serializeElements(elements);
            const res = await updateBuilder(html);
            setMessage({ kind: 'success', text: res.success || 'Saved' });
            setTimeout(() => nav('/admin/certificate'), 600);
        } catch (err) {
            setMessage({ kind: 'error', text: err?.response?.data?.error || 'Save failed' });
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div className="builder-page">
            {/* Imported display fonts (mirrors @import urls in the blade) */}
            ...
            {/* Sidebar */}
            <aside className={`builder-sidebar ${open ? 'open' : ''}`}>
                ...
                <div className="builder-sidebar-body">
                    <div className="builder-card">
                        <h6 className="builder-card-title">Available Variable Data</h6>
                        <div>
                            {VARIABLES.map((v) => (
                                <button
                                    key={v}
                    ...
                    <div className="builder-card">
                        <h6 className="builder-card-title">Add a new element</h6>

                        <div style={{ marginBottom: 12 }}>
                            <label className="builder-form-label" htmlFor="cert_elem_content">
                        ...
                        <div style={{ marginBottom: 12 }}>
                            <label className="builder-form-label">Choice a font-family</label>
                            {FONT_FAMILIES.map((f) => (
                        ...
                        <div style={{ marginBottom: 12 }}>
                            <label className="builder-form-label" htmlFor="cert_font_size">Font Size</label>
                            <input
                                id="cert_font_size"
                                type="number"
                                className="builder-form-control"
                        ...
                        <div style={{ marginBottom: 12 }}>
                            <button type="button" className="builder-btn-light" onClick={addElement} disabled={!newText.trim()}>
                                Add
                            </button>
                        </div>

                        <div style={{ marginBottom: 32 }}>
                            <button type="button" className="builder-btn-primary" onClick={onSave} disabled={submitting}>
                                {submitting ? 'Saving…' : 'Save Template'}
                            </button>
                        </div>

                        {message && (
                            <p style={{ fontSize: 12, color: message.kind === 'success' ? '#169f48' : '#ef3f6e' }}>
                                {message.text}
                            </p>
                        )}
                    </div>
            ...
            {/* Canvas (mirrors div#certificate_builder_content / div#certificate-layout-module) */}
...
// ----------------- (de)serialization -----------------
// Saved HTML envelope matches the PHP-saved one so the download renderer
// (token replacement + QR + html2canvas) keeps working unchanged.
