/**
 * AdminTutorSubjects - Tutor booking subject CRUD.
 *
 * ============================================================================
 * ORIGINAL BLADE:
 *   resources/views/admin/tutor_booking/subjects.blade.php
 *   resources/views/admin/tutor_booking/subject_add.blade.php
 *   resources/views/admin/tutor_booking/subject_edit.blade.php
 * ============================================================================
 *
 * Same shape as AdminTutorCategories — name-only single-field CRUD with status
 * toggle. Kept as a separate component for clarity of routes + role/api prefix.
 */
...
export default function AdminTutorSubjects() {
  ...
  const fetchSubjects = useCallback(async () => {
    setLoading(true);
    try {
      const res = await get(`/api/admin/tutor-booking/subjects?page=${page}`);
  ...
  const save = async () => {
    ...
    try {
      if (modal.id) {
        await post(`/api/admin/tutor-booking/subjects/${modal.id}`, { name: modal.name, _method: 'PUT' });
      } else {
        await post('/api/admin/tutor-booking/subjects', { name: modal.name });
      }
  ...
  const toggleStatus = async (id) => {
    try {
      await post(`/api/admin/tutor-booking/subjects/${id}/toggle-status`);
      fetchSubjects();
    } catch {
  ...
  const handleDelete = async (id) => {
    try {
      await del(`/api/admin/tutor-booking/subjects/${id}`);
  ...
  return (
    <>
        ...
        <div className="py-3 px-5 my-3">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <h4 className="title text-base">
              ...
              {translate('Tutor subjects')} <span className="text-gray-500">({pagination.total})</span>
            </h4>
                        ...
                        {subjects.map((s, i) => (
                          <tr key={s.id}>
                            ...
                            <td className="text-center">
                              {s.status === 1 || s.status === true ? translate('Active') : translate('Deactive')}
                            </td>
                            <td className="text-center">
                              <div className="relative ol-icon-dropdown">
                                ...
                                <ul className="absolute mt-2 bg-white rounded-lg shadow-lg border border-gray-100 py-1 z-30">
                                  <li>
                                    <button className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50" type="button" onClick={() => setModal({ id: s.id, name: s.name })}>
                                      {translate('Edit')}
                                    </button>
                                  </li>
                                  <li>
                                    <button className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50" type="button" onClick={() => toggleStatus(s.id)}>
                                      {s.status === 1 || s.status === true ? translate('Deactive') : translate('Active')}
                                    </button>
                                  </li>
                                  <li>
                                    <button className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 text-red-600" type="button" onClick={() => setConfirm({ id: s.id })}>
                                      {translate('Delete')}
                                    </button>
                                  </li>
                                </ul>
      ...
      <Modal
        ...
        title={translate(modal?.id ? 'Edit subject' : 'Add new subject')}
        footer={(
      ...
      >
        {modal && (
          <>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">{translate('Subject Name')}</label>
            <input
      ...
      <ConfirmModal
        ...
        message={translate('Delete this subject?')}
      />
    </>
  );
}
